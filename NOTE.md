...
使用者搜尋時不會意識到自己在做邏輯運算，同一串關鍵字中經常同時混用 AND 和 OR 的意圖（例如 `台北 拉麵 豚骨 味噌` — 前兩個是交集，後兩個是替代選項）。成熟的搜尋引擎不做呆板的單一邏輯，而是透過相關性排序讓使用者不需要思考這個問題 — 全部匹配的排前面，部分匹配的排後面但不排除。

多數系統名義上預設 AND，但都有放寬機制，依賴排序來平衡精準與召回。在資料量小、無排序的情境下，OR 搭配 keyword highlight 是比較務實的選擇。
